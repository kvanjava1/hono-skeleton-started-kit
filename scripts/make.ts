...
const run = async () => {
  try {
    switch (type) {
      case "migration":
        if (!target || !name)
          throw new Error("Usage: bun run make migration <target> <name>");
      ...
      case "seeder":
        if (!target || !name)
          throw new Error("Usage: bun run make seeder <target> <name>");
      ...
      case "service":
        if (!target || !name)
          throw new Error("Usage: bun run make service <target> <name>");
      ...
      case "repository":
        if (!target || !name)
          throw new Error("Usage: bun run make repository <target> <name>");
...
function createControllerFile(name: string) {
  const fileName = name.toLowerCase().endsWith(".controller")
    ? name
    : `${name}.controller`;
...
function createServiceFile(
  type: "mysql" | "mongo" | "pg" | "sqlite",
  name: string,
) {
  const fileName = name.toLowerCase().endsWith(".service")
    ? name
    : `${name}.service`;
...
function createRepositoryFile(
  ...
  const fileName = name.toLowerCase().endsWith(".repository")
    ? name
    : `${name}.repository`;
...
function createSchemaFile(name: string) {
  const fileName = name.toLowerCase().endsWith(".schema")
    ? name
    : `${name}.schema`;
